...
export default async function AnalyticsPage() {
  ...
  const [hourly, dow, items, receipts] = await Promise.all([
    supabase.rpc('hourly_transaction_counts', { p_merchant_id: mid }),
    supabase.rpc('dow_transaction_counts',    { p_merchant_id: mid }),
    supabase.rpc('top_items',                 { p_merchant_id: mid, p_limit: 10 }),
    supabase
      .from('receipts')
  ...
  ;(receipts.data ?? []).forEach((r) => {
    catMap[r.category] = (catMap[r.category] ?? 0) + Number(r.total)
  })
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
          ...
          <TimeOfDayChart data={hourly.data ?? []} />
        </div>
